...
struct Schematic {
  ...
  void make_part_valid(Part &part) {
    ...
    //   get indices
    rng::transform(part.digits_vec, std::back_inserter(digits), [](auto dig) {
      return std::get<1>(dig) - ZERO_ASCII_OFFSET;
    });
    for (const auto &d : digits) {
      std::cout << "My digits: " << d << '\n';
    }
    int result = 0;
